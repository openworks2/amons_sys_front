...
Dahua.prototype = {
      ...
      WebVideoCtrl.checkPluginInstall()
        .done(function () {
          ...
          WebVideoCtrl.initPlugin("Protocol2", function () {
            ...
            let top = (_this.position.top =
              _this.camOCX.offsetTop + window.outerHeight - window.innerHeight);
        ...
        .fail(function () {
          ...
          const result = window.confirm(
            "플러그인이 설치되지 않았거나 실행되지 않았습니다. 확인을 누르면 WebPlugin.exe 패키지가 다운로드 됩니다.！"
          );
          if (result) {
  ...
  onScrollHandler() {
    ...
    if (document.getElementById(_this.objectId)) {
      let coverInfo =
        document.getElementById(_this.objectId).getBoundingClientRect() || null;

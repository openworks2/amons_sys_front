 ...
 */
export const createPromiseThunkOfPut = (type, promiseCreator, idSelector = defaultIdSelector) => {
    const [SUCCESS, ERROR] = [`${type}_SUCCESS`, `${type}_ERROR`];
        ...
        switch (action.type) {
            ...
            case SUCCESS:
                return {
                    ...
                    [key]: {
                        loading: false,
                        data: state[key].data.map(item => item[id] === action.payload[id] ? action.payload : item),
                        error: null
                    }
                }
            case ERROR:

...
export const getRemainWorkers = async () => {
  const response = await axios.get("/api/ble/bles/input/worker");
  return response.data;
};

// 현재 잔류 차량 조회
export const getRemainVehicles = async () => {
  const response = await axios.get("/api/ble/bles/input/vehicle");
  return response.data;
};
...
export const postRemainVehiclesSearch = async (data) => {
  const response = await axios.post("/api/ble/bles/input/vehicle/search", data);
  return response.data;
};

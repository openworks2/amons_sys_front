...
export const getAnnounces = async () => {
  const response = await axios.get(`${API}/api/announce/announces`);
  return response.data;
};

// INDEX로 조회하는 비동기 함수
export const getAnnounceById = async (index) => {
  const response = await axios.get(`${API}/api/announce/announces/${index}`);
  return response.data;
};
...
export const putAnnounce = async (index, data) => {
  const response = await axios.put(
    `${API}/api/announce/announces/${index}`,
    data
  );
  return response.data;
};

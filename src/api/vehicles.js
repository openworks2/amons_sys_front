...
export const getVehicles = async () => {
  const response = await axios.get(`${API}/api/vehicle/vehicles`);
  return response.data;
};

// INDEX로 조회하는 비동기 함수
export const getVehicleById = async (index) => {
  const response = await axios.get(`${API}/api/vehicle/vehicles/${index}`);
  return response.data;
};

// 데이터 추가를 위한 POST 비동기 함수
export const postVehicle = async (data) => {
  const response = await axios.post(`${API}/api/vehicle/vehicles`, data);
  return response.data;
};
...
export const putVehicle = async (index, data) => {
  const response = await axios.put(
    `${API}/api/vehicle/vehicles/${index}`,
    data
  );
  return response.data;
};

...
const MonitorContainer = () => {
  ...
  const localScanner = (localIndex) => {
    ...
    const filterScanners = data.filter(
      (item) => item.local_index === localIndex
    );
    return filterScanners;
  };
  const localBeacon = (localIndex) => {
    const data = beacon.data;
    const filterBeacons = data.filter(
      (item) => item.local_index === localIndex
    );
  ...
  const targetClick = (e) => {
    ...
    if (
      clzName === "detail-panel-button active" ||
      e.target.closest(".detail-panel-button") ||
      e.target.closest(".table-box")
    ) {
      return;
    } else if (
      clzName === "ptz-control-button active" ||
      e.target.closest(".controll-box")
    ) {
      return;
    } else {
      ...
      >
        <div className="left-compo">
          <div className="left-top-box info-box">
            {monitor.data && beacon.data && (
              <StatusInfo
                localInfo={monitor.data}
                bleData={beacon.data && beacon.data}
              />
            )}
          </div>
        ...
        <div className="right-compo">
            ...
            <div className="top-panel">
              {announceList && environment.data && (
                <NoticeCompo
                  ...
                  rollingCount={environment.data[0].announce_rolling}
                />
              )}
            </div>
              ...
              <div className="top-cctv">
                {monitor.data && (
                  <CctvComponent
                    ...
                    scanner={
                      scanner.data && localScanner(monitor.data[0].local_index)
                    }
                    bleData={
                      beacon.data && localBeacon(monitor.data[0].local_index)
                    }
                    processDisabled={
                      environment.data && environment.data[0].process_disabled
                    }
              ...
              <div className="bottom-cctv">
                {monitor.data && (
                  <CctvComponent
                    ...
                    expandMap={expandMap}
                    data={
                      monitor.data && monitor.data.length > 0 && monitor.data[2]
                    }
                    scanner={
                      scanner.data && localScanner(monitor.data[2].local_index)
                    }
                    bleData={
                      beacon.data && localBeacon(monitor.data[2].local_index)
                    }
                    processDisabled={
                      environment.data && environment.data[0].process_disabled
                    }
          ...
          <div className="right-right-box">
            <div className="top-panel">
              {weather.data && <TodayInfoComponent weather={weather} />}
            </div>
            <div className="cctv-panel">
              <div className="top-cctv">
                {monitor.data && beacon.data && (
                  <CctvComponent
                    ...
                    scanner={
                      scanner.data && localScanner(monitor.data[1].local_index)
                    }
                    bleData={
                      beacon.data && localBeacon(monitor.data[1].local_index)
                    }
                    processDisabled={
                      environment.data && environment.data[0].process_disabled
                    }
              ...
              <div className="bottom-cctv">
                {monitor.data && beacon.data && (
                  <CctvComponent
                    ...
                    scanner={
                      scanner.data && localScanner(monitor.data[3].local_index)
                    }
                    bleData={
                      beacon.data && localBeacon(monitor.data[3].local_index)
                    }
                    processDisabled={
                      environment.data && environment.data[0].process_disabled
                    }
